...
当前第一阶段暂不拆分字幕通道和动作通道，也不提前重写 ReactionScheduler。现有 TTL、单反应约 3 秒、优先级、去重、合并和过期丢弃规则先作为可用基线保留。

原因：提醒、批量任务完成和更多来源事件尚未接入，当前没有足够真实事件量证明队列延迟已经影响使用。下载完成等即时事件未来接入后，需要重点观察“来源应用已经显示完成，但 AIRI 反应仍在排队”的错位问题。

触发后续优化的条件：

- 同时出现多个真实事件，用户能感知到字幕或动作明显滞后；
- 下载完成、任务失败等结果事件被普通进度或低优先级反应阻塞；
- 重复进度没有被有效合并，造成字幕刷屏或动作连续播放；
- 提醒功能接入后，需要区分即时结果、普通提醒和可丢弃状态更新。

届时优先采用“字幕/信息即时通道 + 动作节流队列”的两层方案：信息结果不等待动作，动作按优先级、合并键和新鲜度择机播放。不要简单把所有事件都改成打断，也不要用延长 TTL 掩盖过时消息。
